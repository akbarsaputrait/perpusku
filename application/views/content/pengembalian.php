<body>
			...
			<div class="row mb-5">
				<div class="col-md-12">
					<div class="table-responsive bg-white clr-blck p-4">
						<table cellpadding="1" cellspacing="1" id="daftar_pengembalian" class="display table table-bordered table-hover table-striped"
						width="100%">
							<thead>
								<tr>
	...
	<!-- Buku Kembali -->
	<div class="modal fade" id="pengembalianBuku" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
...
<script>
	...
	$(document).ready(function () {
		...
		// DATATABLE SISWA
		var pengembalian = $('#daftar_pengembalian').DataTable({
			dom: 'ltp',
			select: true,
			lengthChange: true,
			searching: true,
			...
			serverSide: true,
			ajax: {
				url: base_url + "buku/get_pengembalian/",
				type: 'POST'
			},
			order: [
				[0, 'asc'],
				...
				},
				{
					...
					'render': function (data, type, row) {
						return '<button type="button" id="button_show" class="btn btn-success btn-block" data-id="' +
							data +
							'" data-toggle="modal" data-target="#pengembalianBuku"><i class="fas fa-check"></i></button>';
					}
				}
			]
		});

		$(document).on('show.bs.modal', '#pengembalianBuku', function (event) {
			...
			$.ajax({
				type: 'GET',
				url: base_url + 'buku/get_data_pengembalian/' + id,
						...
						$.each(data.data, function (index, value) {
							if (value.telat < 0) {
								$(".data_peminjam").append(
									'<div class="row"><div class="col-md-12 text-center p-4"><h3><span class="badge badge-dark">' +
									value
									.kode_pinjaman +
									'</span></h3><h1>' + value.nama +
									'</h1></h1><h6 class="text-muted">' + value.kelas + ' ' + value.jurusan + '</h6><h4 class="mb-3">"' +
									value.judul +
									'"</h4><div class="row"><div class="col-md-12 d-flex justify-content-center"><h5><span class="badge badge-success">Tidak terlambat.</span></h5></div></div><div class="row"><div class="col-md-6"><h1>' +
									value.tgl_pinjam +
								...
								$(".data_peminjam").append(
									'<div class="row"><div class="col-md-12 text-center p-4"><h3><span class="badge badge-' + badge() +
									'">' +
									value
									.kode_pinjaman +
									'</span></h3><h1>' + value.nama +
									'</h1></h1><h6 class="text-muted">' + value.kelas + ' ' + value.jurusan + '</h6><h4 class="mb-3">"' +
									value.judul +
									'"</h4><div class="row"><div class="col-md-12 d-flex justify-content-center"><h5><span class="badge badge-danger">Terlambat ' +
									value.telat + ' hari.</span></h5></div></div><div class="row"><div class="col-md-6"><h1>' +
									value.tgl_pinjam +
									'</h1><p class="text-small">TANGGAL PINJAM</p></p></div><div class="col-md-6"><h1>' + value.tgl_kembali +
									'</h1><p class="text-small">TANGGAL KEMBALI</p></div></div></div></div><div class="row"><div class="col-md-6 d-flex justify-content-center form-group"><button type="button" id="button_confirm" data-id="' +
									value.id_pengembalian +
		...
		$(document).on('click', '#refresh', function () {
			$(".loader.loader-default.is-active").fadeIn(5);
			pengembalian.ajax.reload(null, false);
		});

		$(document).on('keyup', '#filter', function () {
			pengembalian.search($(this).val()).draw();
		});

		$(document).on('click', '#button_confirm', function () {
			var id_pengembalian = $(this).data('id');
			$(".loader.loader-default.is-active").fadeIn(5);

			$.ajax({
				type: 'POST',
				url: base_url + 'buku/pengembalian_buku/',
				data: {
